...
public class SimpleReportExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/hmsdb","root", "");
		...
		report
		  .columns(
		  	Columns.column("Customer Id", "id_patient", DataTypes.integerType())
		  		.setHorizontalAlignment(HorizontalAlignment.LEFT),
		  	Columns.column("First Name", "patient_name", DataTypes.stringType()),
		  	Columns.column("Last Name", "address", DataTypes.stringType()),
		  	)
		  .title(//title of the report
		  	Components.text("SimpleReportExample")
		  		.setHorizontalAlignment(HorizontalAlignment.CENTER))
		  .pageFooter(Components.pageXofY())//show page number on the page footer
		  .setDataSource("SELECT id_patient, patient_name, address FROM patient", connection);
